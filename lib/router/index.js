...
const makeCustomRes = require('../response')
...
module.exports = {
  ...
  resolve: function(req, res) {
    let app = module.exports.app
    
    res = makeCustomRes(res, app)

    let route = match(req, res, handlers)
    route.handlers.push(defaultHandler)
    ...
    /**
     * Default handler for paths that don't match any route
     * 
     * @param  {Request} req
     * @param  {Response} res
     */
    function defaultHandler(req, res) {
      res.statusCode = 404
      res.end(`Can't ${req.method} ${req.url}`)
    }
  },
